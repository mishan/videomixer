...
dev:  ## Install development dependencies into the current environment
	pip install -r requirements-dev.txt

venv:  ## Create a virtualenv that can see the system PyGObject
...
unit-docker:  ## Run the unit tests inside the container (no host GStreamer)
	docker compose build videomixer
	docker compose run --rm --no-deps -T videomixer \
	  sh -c "pip install -q -r requirements-dev.txt && python3 -m pytest"
